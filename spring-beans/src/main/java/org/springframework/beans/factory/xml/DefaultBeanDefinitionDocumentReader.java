...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		if (absoluteLocation) {
			...
			try {
				int importCount;
				//Resource存在多个子实现类，如VfsResource、FileSystemResource等，而每个resource的createRelative方法实现都不一样，所以这里先使用子类的方法常识解析
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		/**
		 * 首先委托BeanDefinitionParserDelegate类的parseBeanDefinitionElement方法进行元素解析，返回BeanDefinitionHolder
		 * 类型的实例bdHolder，经过这个方法后，bdHolder实例已经包含我们配置文件中配置的各种属性了，例如class,name,id,alias之类的属性
		 * */
