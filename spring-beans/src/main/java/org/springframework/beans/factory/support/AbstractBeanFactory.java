...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		* 也就是将ObjectFactory加入到缓存中，一旦下个bean创建的时候需要上个bean则直接使用ObjectFactory
		* */
		...
		if (sharedInstance != null && args == null) {
			...
			* */
			...
			//如果beanDefinitionMap中也就是在所有已经加载的类中不包括beanName则尝试从parentBeanFatory中检测
			...
			try {
				//将存储XML配置文件的GernerBeanDefinition转换成RootBeanDefinition,如果指定BeanName是子Bean的话同时会合并父类的相关属性
	...
	protected Object getObjectForBeanInstance(
		...
		/*
		* 现在我们有了个bean的实例，这个实例可能会是正常的bean或者是FactoryBean，如果是FactoryBean我们使用它来创建实例，
		* 但是如果用户想要直接获取工厂实例而不是工厂的getObject方法对应的实例那么传入的name应该加入前缀
		* */
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
		...
		if (object == null) {
			...
			//到这里已经明确只掉beanInstance一定是FactoryBean类型
			FactoryBean<?> factory = (FactoryBean<?>) beanInstance;
			// Caches object obtained from FactoryBean if it is a singleton.
			//containBeanDefinition检测beanDefinitionMap中也就是在所有的已经加载的类中检测是否定义beanName
			if (mbd == null && containsBeanDefinition(beanName)) {
				//将存在XML配置文件的GernericBeanDefinition转化成RootBeanDefinition，如果指定BeanName是子Bean的话同时会合并父类的相关属性
